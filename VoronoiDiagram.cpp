...
struct Event {
  ...
  Event(int _index = -1, int _type = Event::SKIP,
...
EventPtr checkCircleEvent(NodePtr n1, NodePtr n2, NodePtr n3,
                          const std::vector<Point2D> &points,
                          double sweepline) {
  if (n1 == nullptr || n2 == nullptr || n3 == nullptr) return nullptr;
  ...
  if (p2.y > p1.y && p2.y > p3.y) return nullptr;

  if (!findCircleCenter(p1, p2, p3, center)) return nullptr;
...
void build_voronoi(const std::vector<Point2D> &points,
  ...
  for (size_t i = 0; i < points.size(); ++i) {
    pq.push(
        std::make_shared<Event>(static_cast<int>(i), Event::SITE, points[i]));
  }
  ...
  while (!pq.empty()) {
    ...
    if (e->type == Event::SITE) {  // handle site event
      ...
      if (root == nullptr) {  // init empty beachline tree
        root = std::make_shared<Node>(std::make_pair(point_i, point_i),
                                      &sweepline, &points);
      } else {  // if it's not empty

        NodePtr arc = find(root, e->point.x);
        NodePtr subtree, left_leaf, right_leaf;
        ...
        int isp_num =
            intersectionPointsNum(points[arc->get_id()], e->point, sweepline);

        // different subtrees depending on the number of intersection points
        if (isp_num == 1) {
          subtree = make_simple_subtree(point_i, arc->get_id(), &sweepline,
                                        &points, halfedges);
          left_leaf = subtree->left;
          right_leaf = subtree->right;
        } else if (isp_num == 2) {
          subtree = make_subtree(point_i, arc->get_id(), &sweepline, &points,
                                 halfedges);
          left_leaf = subtree->left;
        ...
        if (arc->prev != nullptr) connect(arc->prev, left_leaf);

        if (arc->next != nullptr) connect(right_leaf, arc->next);

        // Replace old leaf with a subtree and rebalance it
        root = replace(arc, subtree);

        // Check circle events
        EventPtr circle_event = checkCircleEvent(
            left_leaf->prev, left_leaf, left_leaf->next, points, sweepline);
        if (circle_event != nullptr) {
          pq.push(circle_event);
        }
        circle_event = checkCircleEvent(right_leaf->prev, right_leaf,
                                        right_leaf->next, points, sweepline);
        if (circle_event != nullptr) {
          pq.push(circle_event);
        }
      }
      ...
      NodePtr arc = e->arc, prev_leaf, next_leaf;

      // get breakpoint nodes
      std::pair<NodePtr, NodePtr> breakp = breakpoints(arc);

      // recheck if it's a false alarm 1
      if (breakp.first == nullptr || breakp.second == nullptr) {
        continue;
      }

      // recheck if it's a false alarm 2
      double v1 = breakp.first->value(), v2 = breakp.second->value();
      ...
      VertexPtr vertex = std::make_shared<Vertex>(e->center);
      HalfEdgePtr h_first = breakp.first->edge;
      HalfEdgePtr h_second = breakp.second->edge;
      ...
      NodePtr new_edge_node;
      if (arc->parent == breakp.first)
        new_edge_node = breakp.second;
      else
        new_edge_node = breakp.first;

      // remove arc from the beachline
      root = remove(arc);

      // make a new pair of halfedges
      std::pair<HalfEdgePtr, HalfEdgePtr> twin_nodes =
          make_twins(prev_leaf->get_id(), next_leaf->get_id());
      ...
      if (prev_leaf != nullptr && next_leaf != nullptr) {
        EventPtr circle_event = checkCircleEvent(prev_leaf->prev, prev_leaf,
                                                 next_leaf, points, sweepline);
        if (circle_event != nullptr) {
          pq.push(circle_event);
        }
