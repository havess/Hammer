...
Node::Node(const std::pair<int, int> &_indices, double *_sweepline,
           const std::vector<Point2D> *_points, NodePtr _left, NodePtr _right,
           NodePtr _parent, int _height)
    : indices(_indices),
      left(_left),
      right(_right),
      parent(_parent),
...
double Node::value() {
  if (points == nullptr) return std::numeric_limits<double>::infinity();
  if (is_leaf()) {
 ...
 */
bool is_root(NodePtr node) { return node->parent == nullptr; }
...
NodePtr replace(NodePtr node, NodePtr new_node) {
  ...
  while (parent_node != nullptr) {
    ...
    if (balance >
        1) {  // left subtree is higher than right subtree by more than 1
      if (parent_node->left != nullptr && !parent_node->left->is_leaf() &&
          get_balance(parent_node->left) < 0) {  // @TODO ensure that
        parent_node->left = rotate_left(parent_node->left);
      }
      parent_node = rotate_right(parent_node);
    } else if (balance <
               -1) {  // right subtree is lower than left subtree by more than 1
...
NodePtr remove(NodePtr leaf) {
  ...
  // This function removes the leaf and it's parent corresponding to one
  // breakpoint. It moves up in a tree and rebalaces it. If function encounters
  // second breakpoint, it replaces this breakpoint with a new one. This is
  // possible because when the circle event appears, two breakpoints coincide
  // and thus they should be represented by one.

  if (leaf == nullptr) return nullptr;

  NodePtr parent = leaf->parent, grandparent = parent->parent;
  ...
  NodePtr new_root = grandparent;
  // Go up and rebalance the whole tree
  while (grandparent != nullptr) {
    if (grandparent->has_indices(other_bp))
      grandparent->indices =
          std::make_pair(leaf->prev->get_id(), leaf->next->get_id());
...
std::pair<NodePtr, NodePtr> breakpoints(NodePtr leaf) {
  if (leaf == nullptr || leaf->next == nullptr || leaf->prev == nullptr)
    return std::make_pair<NodePtr>(nullptr, nullptr);

  NodePtr parent = leaf->parent, gparent = leaf->parent;
  std::pair<int, int> bp1(leaf->prev->get_id(),
                          leaf->get_id());  // left breakpoint
  std::pair<int, int> bp2(leaf->get_id(),
                          leaf->next->get_id());  // right breakpoint
  std::pair<int, int> other_bp;
...
NodePtr make_subtree(int index, int index_behind, double *sweepline,
  ...
  NodePtr node1 = std::make_shared<Node>(std::make_pair(index_behind, index),
                                         sweepline, points);
  NodePtr node2 = std::make_shared<Node>(std::make_pair(index, index_behind),
                                         sweepline, points);

  // create leaf nodes
  NodePtr leaf1 = std::make_shared<Node>(
      std::make_pair(index_behind, index_behind), sweepline, points);
  NodePtr leaf2 =
      std::make_shared<Node>(std::make_pair(index, index), sweepline, points);
  NodePtr leaf3 = std::make_shared<Node>(
      std::make_pair(index_behind, index_behind), sweepline, points);
  ...
  // add halfedges
  std::pair<HalfEdgePtr, HalfEdgePtr> twin_edges =
      make_twins(index_behind, index);
...
NodePtr make_simple_subtree(int index, int index_behind, double *sweepline,
  ...
  if ((*points)[index].x < (*points)[index_behind].x) {
    ...
    node = std::make_shared<Node>(std::make_pair(index, index_behind),
                                  sweepline, points);
    leaf_l =
        std::make_shared<Node>(std::make_pair(index, index), sweepline, points);
    leaf_r = std::make_shared<Node>(std::make_pair(index_behind, index_behind),
                                    sweepline, points);
    node->edge = twin_edges.second;  // twin_edges.first;
  } else {
    node = std::make_shared<Node>(std::make_pair(index_behind, index),
                                  sweepline, points);
    leaf_l = std::make_shared<Node>(std::make_pair(index_behind, index_behind),
                                    sweepline, points);
    leaf_r =
        std::make_shared<Node>(std::make_pair(index, index), sweepline, points);
...
bool _validate(NodePtr node) {
  ...
  if (node->is_leaf()) {
    if (node->left != nullptr || node->right != nullptr) {
      std::cout << "LEAF NOT A LEAF: " << node->indices.first << ", "
                << node->indices.second << std::endl;
      return false;
    }
  } else {
    if (node->left == nullptr || node->right == nullptr) {
      std::cout << " BP WITHOUT LEAF: " << node->indices.first << ", "
                << node->indices.second << std::endl;
      return false;
    }
  }
...
bool _check_balance(NodePtr node) {
  ...
  if (_check_balance(node->left) && _check_balance(node->right)) {
    if (fabs(get_balance(node)) > 1) {
      std::cout << "+unbalanced (" << node->indices.first << ", "
                << node->indices.second << ")" << std::endl;
...
/**
 Print tree
 */
void print_tree(NodePtr root, int width) {
  if (root == nullptr) return;

  std::vector<std::vector<NodePtr>> layers(root->height);
